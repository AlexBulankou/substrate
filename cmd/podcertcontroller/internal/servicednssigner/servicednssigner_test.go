...
func newTestImpl(t *testing.T, objs ...runtime.Object) (*Impl, *fake.Clientset, *localca.CA) {
	...
	return NewImpl(kc, pool, newPCRClient(t, kc, certsv1beta1.SchemeGroupVersion.String())), kc, ca
}

// newPCRClient builds the PCR client the signer writes its status through,
// against a cluster that serves exactly one PCR API version.  The fixtures
// here are beta objects, so the suite serves beta and the writes land on the
// same resource the assertions read back.  The stable path is covered by
// TestMakeCertWritesThroughTheServedAPIVersion.
func newPCRClient(t *testing.T, kc *fake.Clientset, groupVersion string) *podcertificate.Client {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("NewClient for %s: %v", groupVersion, err)
	}
	return pcrClient
}
...
func TestMakeCertReportsASigningFailure(t *testing.T) {
	...
	kc := fake.NewSimpleClientset(
		...
		testService("svc-a", corev1.ServiceTypeClusterIP, testLabels),
		pcr)
	impl := NewImpl(kc, &failingPool{createErr: injected}, newPCRClient(t, kc, certsv1beta1.SchemeGroupVersion.String()))
...
func TestDesiredClusterTrustBundlesReportsATrustAnchorFailure(t *testing.T) {
	...
	emptyKC := fake.NewSimpleClientset()
	impl := NewImpl(emptyKC, &failingPool{anchorErr: injected}, newPCRClient(t, emptyKC, certsv1beta1.SchemeGroupVersion.String()))
...
// The signer used to write its issued status straight to the beta resource.
// It now goes through the PCR client, which picks whichever version the
// cluster serves -- so on a cluster serving the stable API the status must
// land on the v1 resource, not on a beta one nobody is watching.
func TestMakeCertWritesThroughTheServedAPIVersion(t *testing.T) {
	pcr := testPCR(t)
	// The stable spec carries the key as a stub PKCS#10 request rather than a
	// bare PKIX key, and MakeCert reads its key material from the beta object
	// it is handed.  This twin exists so the status update has something to
	// write to, so it mirrors the identity fields and nothing else.
	v1PCR := &certsv1.PodCertificateRequest{
	...
	pool := &localca.ConcretePool{CAs: []*localca.CA{ca}, ActiveForSigning: ca.ID}
	impl := NewImpl(kc, pool, newPCRClient(t, kc, certsv1.SchemeGroupVersion.String()))

	if err := impl.MakeCert(context.Background(), pcr); err != nil {
		t.Fatalf("MakeCert: %v", err)
	}

	got, err := kc.CertificatesV1().PodCertificateRequests(testNamespace).Get(
		context.Background(), pcr.Name, metav1.GetOptions{})
	if err != nil {
		t.Fatalf("read back the v1 PCR: %v", err)
	}
	if got.Status.CertificateChain == "" {
		t.Error("the v1 PCR carries no certificate chain — the status went somewhere else")
	}

	// And nothing was written to the beta resource, which on this cluster is
	// not served at all.
	for _, action := range kc.Actions() {
		if action.GetResource().Version == "v1beta1" {
			t.Errorf("signer touched the unserved beta resource: %s %s", action.GetVerb(), action.GetResource())
		}
	}
}
